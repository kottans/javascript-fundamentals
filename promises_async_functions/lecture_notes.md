...
Promise => __pending__ => settlement => __fullfilled__/__rejected__
...
Promise.all([
    ...
    wait(1000),
    wait(0)
]).then(() => console.log('done')) // "done" будет выведено не ранее, чем через 10 секунд (время выполнения самого долгого промиса из переданных)
```
...
```javascript
Promise.resolve(2).finally(() => {}).then(console.log); // 2
Promise.reject(3).finally(() => {}).then(console.log); // 3
```
